...
if __name__ == '__main__':
    ...
    def audio_cb(match):
        if 'http' in match.group(2):
            wrapper = '''<figure class="video_container"><iframe src="{}" {}
            > </iframe></figure>\n\n'''
            style = ' height="600" width="1200" frameborder="0" allowfullscreen="true"'
            return wrapper.format( match.group(2), style)
        else:
    ...
    uu = [x for x in pp_audio.findall(md) if 'http' not in x[1]]
    print(uu)
    if uu != []:
        voices.add(uu )
