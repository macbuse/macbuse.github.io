## Sophie Germaines identity


**Sophie Germain's identity** is an algebraic factorization formula that states:

$$a^4 + 4b^4 = (a^2 + 2b^2 + 2ab)(a^2 + 2b^2 - 2ab)$$

It expresses the sum of a fourth power and four times another fourth power as the product of two quadratic polynomials.

### Unusual factorization

of $a^4 + 4b^4$ is a special case of the identity normally it's a
difference of powers that factorizes nicely, but here we have a sum
of powers. The identity is particularly useful in number theory and
algebra for proving that certain numbers are composite.

$$x^n - y^n = (x - y)(x^{n-1} + x^{n-2}y + \cdots + xy^{n-2} +
y^{n-1})$$
...
The proof relies on completing the square:

1. Start with $a^4 + 4b^4$.
2. Add and subtract the middle term $4a^2b^2$:

$$a^4 + 4a^2b^2 + 4b^4 - 4a^2b^2$$


3. Rewrite the first three terms as a perfect square:

$$(a^2 + 2b^2)^2 - (2ab)^2$$


4. Apply the difference of two squares formula ($x^2 - y^2 = (x-y)(x+y)$):
...
So let's set $z = a/b$ then we have:

$$(z^4 + 4) b^4 = (z^2 + 2 + 2z)(z^2 + 2 - 2z) b^4$$
$$(z^4 + 4)  = (z^2 + 2z + 2)(z^2 + - 2z + 2) $$
$= ((z + 1)^2  + 1)((z - 1)^2  + 1)$

It's funny because this looks like multiplication in the class group (which is trivial).

The roots on the right are $\pm(1 \pm i)$ and similarly on the left.
Note that the roots form a square in the complex plane, and the
factorization reflects this symmetry.
Likewise for the roots $\pm 1, \pm i$ one has 
$$z^4 -1 = (z^2 + 1)(z^2 - 1)$$
...
* **Primes and Factorization:** It shows that any integer of the form $n^4 + 4^k$ (for integer $n > 1$ and $k \ge 1$) is composite. A classic contest problem involves showing $n^4 + 4^n$ is composite for all $n > 1$.
* **Elementary Number Theory:** Frequently used in math olympiads to prove that certain large numbers are not prime without needing explicit division.
