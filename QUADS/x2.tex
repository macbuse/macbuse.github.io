...
We will show that the questions concerning the values of the quadratic form $x^2
+ m y^2$ can be interpreted of terms of arcs on the surface
$\mathbb{H}/\Gamma^t_0(m)$ where
$\ah$ is the 
\textit{anti Hecke congruenc egroup}:
...
\im w = \im  \left (\fracai +b}{ci+d } \right)
= \frac{\im i} {c^2 + d^2},
\end{equation}
...
Suppose that $a,b,c,d,m\in \fp$ and that they satisfy the relations
...
\begin{eqnarray*}
	...
	+ b^2(mc^2)  - 2m(abcd)
\end{eqnarray*}
using (\ref{determinant}) and adding these:
$$
...
Let $XX$ denote the set of $p-1$ arcs joining $\infty$ and $\frac{mk}{p}$
where $1\leq k < p$.
...
This observation allows us to define maps on $XX$ as follows.
The map 
...
this is an element of $XX$.
Thus we may define a map $XX  \rightarrow XX $ that takes the arc $\infty, \frac{mk}{p}$ to $\infty,
	\frac{k'}{p}$.
Note further that 
$$
\begin{pmatrix}
...
$z\mapsto \frac{n}{\bar{z}}$ for $n=2,3$.
This is an inversion in a semi circle with endpoints $\pm \sqrt{n}
\in \mathbb{R}$ .
If $\frac{a}{b}\in \mathbb{Q}$ then it is the endpoint of exactly
one  arc is invariant under this involution and the
of this arc is $\lambda$-length $|a^2 - n b^2|$.
For $n=2$ one can show 
