...
Our argument now is that if any pair of  bicuspidal geodesic $\gamma, \gamma'$
which are invariant under the elliptic involution and have $\lambda$-length a
prime $p$ are related by an automorphism. This is because $p$ splits in an
essentially unique way over the Gaussian integers. This means that there are at
most six such geodesics on $\xx$. Now if $\gamma$ is disjoint from a simple
closed geodesic $\alpha$ then any other such geodesic of the same length is the
image of $\alpha$ under some automorphism of $\xx$. (see discussion of
Conjecture 3 \cite{mcp} above).
